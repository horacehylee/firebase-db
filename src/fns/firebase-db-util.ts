...
export const removeEmptyProp = (input: any): any => {
  const output = { ...input };
...
export const convertDeepObjectOfType = <T>(obj, constructor: { new (): T }) => (
  callback: (objValue: T) => any
) => {
  if (obj == null) {
    return obj;
  }
...
export const convertDeepObject = (obj, predicate: (obj) => boolean) => (
  callback: (obj) => any
) => {
  if (obj == null) {
    return obj;
