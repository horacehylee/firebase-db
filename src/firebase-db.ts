...
const _parseSnapshot = (snapshot: admin.database.DataSnapshot) => FirebaseDbParser.parse(snapshot.val());
...
export class FirebaseDb {
    ...
    static insertWithId(pathValue: string, id: string, data: any): Promise<string> {
        notNull(data, 'data');
        notNull(pathValue, 'pathValue');
        notNull(id, 'id');
    ...
    static get(pathValue: string, id: string): Promise<any> {
        ...
        return ref.once('value').then((snapshot: admin.database.DataSnapshot) => {
            if (!snapshot.val()) {
                ...
                return Promise.reject(new ReferenceError(`unknown id(${id}) for ref(${pathValue})`))
            }
    ...
    static updateFields(pathValue: string, id: string, fields: object): Promise<void> {
        notNull(id, 'id');
        notNull(pathValue, 'pathValue');
    ...
    static updateFieldsAndGet(pathValue: string, id: string, fields: object): Promise<any> {
